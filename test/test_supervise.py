...
from honcho.core.system import (
        TopSample, MemSample, CPUSample,
        LoadAverageSample, ProcessSample, DiskUsageSample)
import honcho.tasks.supervise as supervise
from honcho.config import DIRECTORIES_TO_MONITOR, DATA_TAGS
...
def test_execute(mocker):
    ...
    mocker.patch(
            'honcho.tasks.supervise.get_directory_sizes',
            directory_sizes_mock)
    ...
    mocker.patch(
            'honcho.tasks.supervise.get_measurement_counts',
            measurement_counts_mock)
    ...
    load_average = LoadAverageSample(
        load_1=1, load_5=2, load_15=3,
        unknown_1=4, unknown_2=5
    )
    processes = ProcessSample(
        pid=1, ppid=2, user=3,
        stat=4, vsz=5, mem=6, cpu=7, command=8
    )
    ...
    get_disk_usage_mock.return_value = [DiskUsageSample(
