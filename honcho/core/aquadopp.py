...
def read_aquadopp(ID):
    ...
    try:
        ...
        printf(
            "Imm could not connect to aquadopp. "
            "Line not captured or connectivity error."
        )
    else:
...
def clean_data(ID):
    ...
    try:
        ...
        numbers = aquadopp_raw_data[
            aquadopp_raw_data.find("'>")
            + 3 : aquadopp_raw_data.find("</SampleData")  # noqa
            - 2
        ]
        ...
        with open(
            "/media/mmcblk0p1/logs/aquadopp" + str(ID) + "_raw.log", "a+"
        ) as rawfile:
            rawfile.write("AD" + str(ID) + ":" + aquadopp_data + "\n")
    except Exception:
        printf(
            "Imm did not take data from the aquadopp unit "
            + str(ID)
...
def labeled_data(ID):
    ...
    for i in range(len(aquadopp_data)):
        with open(
            "/media/mmcblk0p1/logs/aquadopp" + str(ID) + "_clean.log", "a+"
        ) as data_file:
...
def aquadopp_sbd():
    ...
    try:
        if unit == "A":
            # When deploying box A - use these ID's when starting deployment
            # files for two aquadopps
            lastline1 = prep_sbd("20")
            lastline2 = prep_sbd("21")
            return lastline1, lastline2
        elif unit == "B":
            # When deploying box B - use these ID's when starting deployment
            # files for two aquadopps
            lastline1 = prep_sbd("22")
            lastline2 = prep_sbd("23")
            return lastline1, lastline2
        elif unit == "C":
            # When deploying box C - use these ID's when starting deployment files
            # for four aquadopps
            lastline1 = prep_sbd("20")
            lastline2 = prep_sbd("21")
