            ...
            if len(prefix) + len(usage) > text_width:
                ...
                def get_lines(parts, indent, prefix=None):
                    ...
                    if line:
                        lines.append(indent + " ".join(line))
                    if prefix is not None:
                        lines[0] = lines[0][len(indent) :]  # noqa
                    return lines
    ...
    def _parse_known_args(self, arg_strings, namespace):
        ...
        for mutex_group in self._mutually_exclusive_groups:
            ...
            for i, mutex_action in enumerate(mutex_group._group_actions):
                ...
                conflicts.extend(group_actions[i + 1 :])  # noqa
        ...
        def consume_positionals(start_index):
            ...
            for action, arg_count in zip(positionals, arg_counts):
                args = arg_strings[start_index : start_index + arg_count]  # noqa
                start_index += arg_count
                take_action(action, args)

            # slice off the Positionals that we just parsed and return the
            # index at which the Positionals' string args stopped
            positionals[:] = positionals[len(arg_counts) :]  # noqa
            return start_index
            ...
            if action not in seen_actions:
                if action.required:
                    ...
                    if (
                        ...
                        and isinstance(action.default, basestring)  # noqa
