...
{   
          ...
          fp1 = fopen("/mnt/weather/weather","w");
          if (fp1 < 0)
          {
          perror(FILEOPEN1);
//		system("/mnt/gpio/radar_OFF");
          ...
          fp2 = fopen("/mnt/weather/weatherdata","a");
          if (fp2 < 0)
          {
//		system("/mnt/gpio/radar_OFF");
          perror(FILEOPEN2);
...
sscanf(sentence,"0R0,Dm=%dD,Sm=%fM,Ta=%fC,Ua=%fP,Pa=%fH,Vs=%fV",&dir, &speed, &temp,&humidity,&pressure,&volts) ;
...
int parse_sentence_0R0(char *sentence) { 	if(strncmp(sentence, "0R0" , 3) == 0)
	return 0; 
	else
	return -1;	 }
