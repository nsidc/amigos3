...
/* ********************************************************************** */
int get_i2c_data_tilt()
{
		
	int return_code, internal_register, LSBX_axis, MSBX_axis, MSBY_axis, LSBY_axis, X_axis , Y_axis ;
	int x_offset, y_offset;
 
        float tilt, tilt_x ; 
  
	internal_register = LSBX_axis = MSBX_axis = MSBY_axis = LSBY_axis = Y_axis = X_axis = 0 ;
	x_offset = 1530 ;
	y_offset = 1530 ; 
...
// printf("first cycle\n");
   return_code = I2C_out_byte(0x20) ; //memsic address is 0x20
   	if (return_code)
   		printf("I2C_out_byte command returned: %d\n", return_code);
// printf("second cycle\n"); 
   return_code = I2C_out_byte(0x00) ; //target address of internal register
   	if (return_code)
   		printf("I2C_out_byte command returned: %d\n", return_code);
// printf("third cycle\n");
   return_code = I2C_out_byte(0x00) ; //set address 0 to 0 ie powerup
   	if (return_code)
   		printf("I2C_out_byte command returned: %d\n", return_code);
I2C_stop() ;

delay(400) ; // allow memsic to wake up this value gives a delay of about 90mS


I2C_start() ;

// printf("fourth cycle\n");
   return_code = I2C_out_byte(0x21) ; //memsic address is x20
   	if (return_code)
   		printf("I2C_out_byte command returned: %d\n", return_code);
...
// tenth cycle 
   ...
   //if (return_code)
   	// printf("I2C_in_byte_rr_first returned: %d\n", return_code);
//   SendAck();
	LSBY_axis = return_code ;
//	printf("LSBY_axis = %d\n", return_code);

X_axis = (MSBX_axis * 256) + LSBX_axis - x_offset ;
Y_axis = (MSBY_axis * 256) + LSBY_axis - y_offset ;

printf("Y_axis = %d\n", X_axis);
printf("X_axis = %d\n", Y_axis);

// calculate angle for vertical mount
tilt = (float)Y_axis / (float)X_axis ;
