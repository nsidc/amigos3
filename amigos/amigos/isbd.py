...
class Isbdmsg:
    ...
    def unpack(self):
        ...
        if self.entire_isbd_msg:
            # Or `self.total_msg_len` might work. Should check that.
            l = len(self.entire_isbd_msg)
            packformat = '>cHcHIccccccccccccccccHHIcHccHcHIcH' + 'c'*(l-51)
            # m is message component list.
            m = list(struct.unpack(packformat, self.entire_isbd_msg))
            for n in [0, 2, 20, 24, 32]:  # For these fields...
                # ...fix binary coded numbers to be proper numbers.
                m[n] = binascii.hexlify(m[n])
            for n in [26, 27, 29]:
                # Byte to int values for location orient and degrees
                m[n] = ord(m[n])
        else:
        ...
        # Time Iridium sees msg (not unit generated or arrival). Seconds since epoch.
        self.msg_timestamp = m[23]
        ...
        if m[34]:
            ...
            self.payload = ''.join(m[34:])
            # Printable hex _string_ of payload.
            self.payload_hex = binascii.hexlify(self.payload)
        else:
            self.payload = None
    ...
    def errors_check_parts(self):
        ...
        if not self.msg_protocol_ver == '01':
            w.append('ISBD Message Protocol Version should be 1, value is: %s' %
                     self.msg_protocol_ver)
        if not self.mo_header_iei == '01':
            w.append(
                'ISBD Message Originate Information Element Identifier should be 1, value is: %s' % self.mo_header_iei)
        if not self.mo_header_len == 28:
            w.append(
                'ISBD Message Originate Header Length should be 28, value is: %s' % self.mo_header_len)
        if not self.mtmsn == 0:
            w.append(
                'ISBD Mobile Terminated Message Seqence Number is not 0 (should only be MO), value is: %s' % self.mtmsn)
        if not self.payload_header_iei == '03':
            w.append('ISBD Payload Header Code is not 3, value is: %s' %
                     self.payload_header_iei)
        if not self.payload_iei == '02':
            w.append(
                'ISBD Message Payload Information Element Identifier should be 2, value is: %s' % self.payload_iei)
        if len(w):
            return '\n'.join(w)
        else:
            return None
    ...
    def log_entry(self):
        ...
        # International Mobile Equipment ID Number (IMEI)
        o += "I=%s," % self.imei
        o += "S=%s," % self.status        # Session status code
        # Timestamp encoded in message (not server arrival)
        o += "T=%s," % self.timestamp_fmt('log')
...
def module_unit_test():
    ...
    O = Isbdmsg()
    O.read_sbd_file(
        '/home/isbd/data/received/20161105/300234063250980-20161105180205-021292.sbd')
    O.read_sbd_file('poo')
