...
class Callback(object):
    ...
    def RxReceived(self, mtmsn, data):
        pass

    def RxMessageQueue(self, count):
        pass
...
class Iridium(object):
    ...
    def __init__(self, portId, baud=9600, callback=None):
        self.s = None
        self.portId = portId
        self.callback = callback
        # When autoSession is True, we'll automatically initiate additional sessions if
        # there are more messages to download.
        self.autoSession = True

        self.s = serial.Serial(self.portId, baud, timeout=5)
    ...
    def messageCheck(self):
        ...
        if self.callback is not None and callable(self.callback.RxStarted):
            self.callback.RxStarted()
        ...
        if self._attemptConnection() and self._attemptSession():
            ...
            if self.callback is not None and callable(self.callback.RxFailed):
                self.callback.RxFailed()
    ...
    def sendMessage(self, msg):
        ...
        if self.callback is not None and callable(self.callback.TxStarted):
            self.callback.TxStarted()
        ...
        if self.callback is not None and callable(self.callback.TxFailed):
            self.callback.TxFailed()
    ...
    # Make sure you DISCONNECT RockBLOCK from power
    # for a few minutes after this command has been issued...
    def setup(self):
        self._ensureConnectionStatus()
        ...
        if self.s.readline().strip() == command and self.s.readline().strip() == 'OK':
            ...
            if (
                self.s.readline().strip() == command
                and self.s.readline().strip() == 'OK'
            ):
        ...
        while True:
            ...
            if self.s.readline().strip() == command:
                ...
                if response.find('+SBDI:') >= 0:
                    ...
                    if moStatus <= 4:
                        self._clearMoBuffer()
                        if self.callback is not None and callable(
                            self.callback.TxSuccess
                        ):
                            self.callback.TxSuccess(moMsn)
                        pass
                    else:
                        if self.callback is not None and callable(
                            self.callback.TxFailed
    ...
    def _attemptConnection(self):
        ...
        while True:
            if TIME_ATTEMPTS == 0:
                if self.callback is not None and callable(self.callback.SignalFail):
                    self.callback.SignalFail()
        ...
        while True:
            ...
            if SIGNAL_ATTEMPTS == 0 or signal < 0:
                ...
                return False
            self.callback.SignalUpdate(signal)
            if signal >= SIGNAL_THRESHOLD:
                if self.callback is not None and callable(self.callback.SignalPass):
                    self.callback.SignalPass()
    ...
    def _processMtMessage(self, mtMsn):
        ...
        if response == 'OK':
            ...
            if self.callback is not None and callable(self.callback.RxReceived):
                self.callback.RxReceived(mtMsn, '')
        else:
            content = response[2:-2]
            if self.callback is not None and callable(self.callback.RxReceived):
                self.callback.RxReceived(mtMsn, content)
            self.s.readline()  # BLANK?
