...
from honcho.config import KEEP_AWAKE, MAX_SYSTEM_SLEEP, WATCHDOG_DEVICE
...
def eth_phy_on():
    with open("/sys/class/gpio/physw/data", "wb") as f:
        f.write(hex(0))
...
def rtsctl_on():
    with open("/sys/class/gpio/rtsctl/data", "wb") as f:
        f.write(hex(1))
